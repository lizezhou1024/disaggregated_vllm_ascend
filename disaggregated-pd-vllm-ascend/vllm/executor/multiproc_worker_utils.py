...
class ProcessWorkerWrapper:
    ...
    def __init__(self, result_handler: ResultHandler,
        ...
        self.process: BaseProcess = self.mp.Process(  # type: ignore[attr-defined]
            ...
            daemon=True)
        # Set ASCEND_RT_VISIBLE_DEVICES to correct value
        origin_rank = int(os.getenv("ASCEND_RT_VISIBLE_DEVICES", "0"))
        os.environ["ASCEND_RT_VISIBLE_DEVICES"] = str(rank+origin_rank)
        self.process.start()
        os.environ["ASCEND_RT_VISIBLE_DEVICES"] = str(origin_rank)
