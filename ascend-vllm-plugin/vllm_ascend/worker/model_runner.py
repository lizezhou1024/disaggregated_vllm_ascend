...
class ModelInputForNPUWithSamplingMetadata(ModelInputForNPU):
    ...
    peer_kv_ranks: Optional[List[int]] = None
    ...
    def as_broadcastable_tensor_dict(self) -> Dict[str, Any]:
        tensor_dict = {
            ...
            "request_ids_to_seq_ids": self.request_ids_to_seq_ids,
            "finished_requests_ids": self.finished_requests_ids,
            "peer_kv_ranks": self.peer_kv_ranks,
        }
        _add_attn_metadata_broadcastable_dict(tensor_dict, self.attn_metadata)
...
class ModelInputForNPUBuilder(ModelRunnerInputBuilderBase[ModelInputForNPU]):
    ...
    def add_seq_group(self, seq_group_metadata: SequenceGroupMetadata):
        ...
        self.peer_kv_ranks.append(seq_group_metadata.peer_kv_rank)
...
class NPUModelRunner(NPUModelRunnerBase[ModelInputForNPUWithSamplingMetadata]):
    ...
    def execute_model(
        ...
        **kwargs,
        ...
        #print(f"[{torch.distributed.get_rank()}] model_runner: in execute_model", flush=True)
        if num_steps > 1:
        ...
        #print(f"[{torch.distributed.get_rank()}]model_runner: end execute_model", flush=True)
    ...
    def need_send_kv(self, model_input, kv_caches) -> bool:
        ...
        if self.vllm_config.kv_transfer_config is None:
            #print(f"kv_transfer_config is none.", flush=True)
            return False
